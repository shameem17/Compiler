...
%token  PRIME LOOP SHOW COMMENT RLOOP WHILE INC DEC CIN
%token ADD SUB MUL DIV POW MOD SQRT GCD LCM FIB MC
%nonassoc IFX
%nonassoc ELSE
%nonassoc SWITCH
%nonassoc CASE
...
statement: ';'			
	...
	|MC         { printf("//Multiple line comment\n");}
	|BREAK       { break;}
	...
	|SWITCH '(' expression ')' START BLOCK  END    {tem=$3;}
...
BLOCK   : C
        ...
        ;
C   : C ';' C
	| CASE DATA ':' expression ';' BREAK ';' { if($2==tem){printf("Case %d executed and the expression value is=%d\n",$2,$4);f=0;}}
	;
D   : DEFAULT ':' expression ';' BREAK ';' {if(f){printf("Default Case executed and the expression value=%d\n",$3);}}
